...
public class CompatibilityTest {
    ...
    public void t05() {
        ...
        List<Map<Integer, String>> list = EasyExcel.read(TestFileUtil.getPath() + "compatibility/t05.xlsx").sheet()
                .doReadSync();
        log.info("data:{}", JSON.toJSONString(list));
    ...
    public void t07() {
        ...
        List<Map<Integer, Object>> list = EasyExcel.read(TestFileUtil.getPath() + "compatibility/t07.xlsx")
                .readDefaultReturn(ReadDefaultReturnEnum.ACTUAL_DATA).sheet().doReadSync();
        log.info("data:{}", JSON.toJSONString(list));
        Assertions.assertEquals(0, new BigDecimal("24.1998124").compareTo((BigDecimal) list.get(0).get(11)));
        
        list = EasyExcel.read(TestFileUtil.getPath() + "compatibility/t07.xlsx").sheet().doReadSync();
        log.info("data:{}", JSON.toJSONString(list));
    ...
    public void t08() {
        ...
        List<Map<Integer, Object>> list = EasyExcel.read(file).readCache(new Ehcache(null, 20)).sheet().doReadSync();
    ...
    public void t09() {
        ...
        List<Map<Integer, Object>> list = EasyExcel.read(file).headRowNumber(0).sheet().doReadSync();
        log.info("data:{}", JSON.toJSONString(list));
        Assertions.assertEquals(1, list.size());
