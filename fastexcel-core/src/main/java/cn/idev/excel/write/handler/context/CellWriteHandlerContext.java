...
public class CellWriteHandlerContext {
    ...
    /**
     * Nullable. It is null in the case of add header. In the case of write there must be not null. firstCellData ==
     * cellDataList.get(0)
     */
    ...
    public CellWriteHandlerContext(WriteContext writeContext, WriteWorkbookHolder writeWorkbookHolder,
            WriteSheetHolder writeSheetHolder, WriteTableHolder writeTableHolder, Row row, Integer rowIndex, Cell cell,
            Integer columnIndex, Integer relativeRowIndex, Head headData, List<WriteCellData<?>> cellDataList,
            WriteCellData<?> firstCellData, Boolean head, ExcelContentProperty excelContentProperty) {
